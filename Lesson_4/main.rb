...
private
  ...
  def create_train
    ...
    number = gets.to_i
    return puts (INVALID_INDEX) if train_type.nil? || number.nil?
  ...
  def del_station_from_route
    list_routes
    puts CHOOSE_NUMBER_ROUTE
    @route = select_from_array(@routes) #не смог обойтись без инстанс переменной, использую @route
  ...
  def create_wagon
    ...
    number = gets.to_i
    return puts (INVALID_INDEX) if wagon_type.nil? || number.nil?
    @wagons << wagon_type.new(number)
    puts WAGON_CREATED
  end
