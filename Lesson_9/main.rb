...
class Main
  ...
  def choice(num)
    ...
    when 14 then wagons_info
    when 15 then control_wagon
    when 16 then abort 'Have a nice day!'
    end
  end
  ...
  def create_wagon
    ...
    wagon_type =
      case gets.to_i
      when 1 then PassangerWagon
      when 2 then CargoWagon
      end
    if wagon_type == PassangerWagon
      puts ENTER_SEATS
  ...
  def control_wagon
    ...
    if choosen_wagon.type == :passanger
      ...
      puts "Номер: #{choosen_wagon.number} Тип: #{choosen_wagon.type}\
      Общих мест: #{choosen_wagon.total_volume}\
      Кол-во занятых: #{choosen_wagon.occupied_volume}"
    elsif choosen_wagon.type == :cargo
      puts ENTER_VOLUME_PLACE
      volume = gets.to_f
      choosen_wagon.take_volume(volume)
      puts "Номер: #{choosen_wagon.number} Тип: #{choosen_wagon.type}\
      Общий объем: #{choosen_wagon.total_volume}\
      Занятый объем: #{choosen_wagon.occupied_volume}"
    end
  rescue RuntimeError => e
    puts e.message
    retry
  ...
  def list_wagons
    @wagons.each_with_index do |wagon, index|
      puts "#{index + 1} Номер: #{wagon.number} Тип: #{wagon.type}"
    end
  end
  ...
  def wagons_info
    @wagons.each.with_index(1) do |wagon, index|
      case wagon
      when PassangerWagon then passenger_wagon_info(wagon, index)
      when CargoWagon     then cargo_wagon_info(wagon, index)
      end
    end
  end

  def passenger_wagon_info(wagon, index)
    puts "#{index + 1}) Номер: #{wagon.number} Тип: #{wagon.type}\
    Общее кол-во мест: #{wagon.total_volume}\
    Кол-во занятых: #{wagon.occupied_volume}\
    Свободных мест: #{wagon.free_volume}"
  end

  def cargo_wagon_info(wagon, index)
    puts "#{index + 1}) Номер: #{wagon.number} Тип: #{wagon.type}\
    Общий объём: #{wagon.total_volume}\
